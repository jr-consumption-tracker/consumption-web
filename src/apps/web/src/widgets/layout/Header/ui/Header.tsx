...
export const Header = () => {
  ...
  const { mobileOpen, toggleMobileMenu, closeMobileMenu } = useMobileMenu();
  ...
  return (
    <>
      <header className="fixed top-0 inset-x-0 z-50">
        <div
          className={cn(
            "relative transition-[background-color,border-color,box-shadow,padding] duration-500 py-4",
            scrolled
      ...
      {/* Mobile Menu — outside header to avoid fixed-in-transform bug */}
      <MobileMenu isOpen={mobileOpen} scrolled={scrolled} onClose={closeMobileMenu} />
    </>
  );
};
